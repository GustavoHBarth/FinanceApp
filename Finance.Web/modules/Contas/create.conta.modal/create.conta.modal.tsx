...
import { CreateContaRequest, EnumCategoriaConta, EnumStatusConta, EnumRecorrencia, CategoriaContaOptions, StatusContaOptions, RecorrenciaOptions } from '@/backend/dto';
import {
	FormContainer,
	FormSection,
	SectionTitle,
	...
	CancelButton,
	SubmitButton
} from './create.conta.modal.style';

interface CreateContaModalProps {
	isOpen: boolean;
	onClose: () => void;
	onSubmit: (data: CreateContaRequest) => Promise<boolean>;
}

export default function CreateContaModal({ isOpen, onClose, onSubmit }: CreateContaModalProps) {
	const [formData, setFormData] = useState<CreateContaRequest>({
		titulo: '',
		descricao: '',
		valor: 0,
	...
	const handleSubmit = async (e: React.FormEvent) => {
		...
		console.log('Modal: handleSubmit iniciado');
		...
		if (validateForm()) {
			...
			try {
				...
				console.log('Modal: Tipos dos dados:', {
					...
					data: typeof dadosFinais.data,
					categoria: typeof dadosFinais.categoria,
					status: typeof dadosFinais.status,
					recorrencia: typeof dadosFinais.recorrencia,
					ehParcelado: typeof dadosFinais.ehParcelado
				});
